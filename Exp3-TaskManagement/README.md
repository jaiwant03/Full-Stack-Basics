...
├── www/                     # Web application source directory (webDir)
│   ├── assets/
│   │   ├── favicon.svg      # Vector branding favicon
│   │   └── icons/
│   │       └── app-icon.svg # 512x512 vector icon for native Android APK
│   ├── index.html           # Core semantic HTML5 application shell & view templates
│   ├── script.js            # Modular Vanilla JS application logic & state engine
│   └── style.css            # Clean modern CSS3 design system & themes
│
├── capacitor.config.json    # Capacitor configuration file
├── package.json             # Dependencies and scripts (sync, open)
├── package-lock.json        # Exact dependency tree
├── .gitignore               # Git ignore rules for node_modules and build files
└── README.md                # Complete documentation & build guide
```
